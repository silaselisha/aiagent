...
// override previous stub with a meaningful one
...
func (f fakeXIngest) GetUserTweets(ctx context.Context, userID string, limit int) ([]model.Tweet, error) {
    // one original tweet to generate quotes
    return []model.Tweet{{ID: "orig1", AuthorID: userID, CreatedAt: time.Now().UTC().Add(-2 * time.Hour)}}, nil
}
func (f fakeXIngest) GetQuoteTweets(ctx context.Context, tweetID string, limit int) ([]model.Tweet, error) {
    if tweetID == "orig1" {
        return []model.Tweet{{ID: "q1", AuthorID: "q", CreatedAt: time.Now().UTC()}}, nil
    }
    return nil, nil
}
...
func TestIngestEngagements_InsertsRetweetAndQuote(t *testing.T) {
    db, err := sqlitevec.Open(":memory:")
    if err != nil { t.Fatal(err) }
    defer db.Close()
    ctx := context.Background()
    ...
    quotes, err := db.LoadEventsRange(ctx, start, end, "quote")
    if err != nil { t.Fatal(err) }
    if len(quotes) == 0 { t.Fatalf("expected quote events") }
}
